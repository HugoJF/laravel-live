...
var users = [];
// var token = window.location.pathname.split('/chat/token/')[1];
var token = authToken;
console.log(authToken);
var socket = io.connect('//' + window.location.host + ':1337', {query: 'token=' + token});
var isActive = true;
var modalOpen = false;
var sounds = false;
var lastUser = '';
...
$('#csgo_room').click(function () {
    socket.emit('changeRoom', {room: 'csgo' });
});

$('#dota2_room').click(function () {
    socket.emit('changeRoom', {room: 'dota2' });
});

$('#hearthstone_room').click(function () {
    socket.emit('changeRoom', {room: 'hearthstone' });
});
